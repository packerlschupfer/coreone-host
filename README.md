...
- For a first-run password via `--ask-pass`: **`sshpass`** on the control node
  (`sudo apt install sshpass`) — not installed by default on Debian/Ubuntu/WSL
...
3. **Run it:**
   ...
   ```
   First run may need `--ask-pass` (which needs `sshpass` on the control node —
   `sudo apt install sshpass`); the `base` role installs your SSH key so later
   runs are passwordless. On WSL2, or if `ansible -m ping` can't reach the Pi by
   name, see **Control node notes** below.
4. **Verify:**
   ```bash
...
## Control node notes (WSL2 / mDNS — or just run it on the Pi)

- **`.local` (mDNS) does not resolve from WSL2** or some corporate/VPN networks. WSL2 has
  its own network namespace and its DNS points at the Windows proxy, which does no mDNS —
  so `klipper.local` fails and the symptom is a *misleading* "can't see my LAN devices"
  (it's name resolution, not routing). Fixes: use the Pi's **IP** in `inventory.ini` (the
  default), or on Windows 11 22H2+ add `networkingMode=mirrored` under `[wsl2]` in
  `%UserProfile%\.wslconfig` and run `wsl --shutdown`.
- **`--ask-pass` requires `sshpass`** on the control node (`sudo apt install sshpass`) —
  not installed by default on Debian/Ubuntu/WSL.
- **Simplest path — run the playbook _on the Pi itself_** (no control node, no WSL, no
  name resolution to debug):
  ```bash
  # on the Pi:
  sudo apt install -y ansible git
  git clone <this-repo-url> && cd ansible_klipper
  cp host_vars/klippy.yml.example host_vars/klippy.yml   # edit lan_subnet, mcu_serial
  ansible-playbook site.yml -i inventory.ini -c local -l klippy
  ```
  `-c local` (ansible_connection=local) runs every task locally against the Pi, so the
  control-node network stack never enters the picture (`ansible_host` is then irrelevant).
